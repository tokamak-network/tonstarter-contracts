    ...
    IPublicSale
{
    ...
    */
        ...
        returns (uint256)
    {
        ...
        uint256 difftime = block.timestamp.sub(startClaimTime);
        //console.log("calculClaimAmount: startClaimTime  %s  , difftime: %s, claimInterval: %s ", startClaimTime, difftime, claimInterval);
        uint256 reward = realSaleAmount.div(claimPeriod);

        uint256 period = (difftime / claimInterval).add(1);
        if(period >= claimPeriod){
            reward = realSaleAmount.sub(userClaim.claimAmount);
        }
        //console.log("calculClaimAmount: period %s, claimInterval: %s  , reward: %s ", period, claimInterval, reward);

        return reward;
    }
/*
    function calculClaimAmountTransaction(address _account)
        public
        returns (uint256)
    {
        require(
        ...
        console.log("calculClaimAmount: block.timestamp  %s  ",block.timestamp);
        ...
        console.log("calculClaimAmount: period %s, claimInterval: %s , reward: %s, claimPeriod: %s ", period, claimInterval, reward, claimPeriod);
    ...
    */
    ...
    function openSaleUserAmount(address user) public view returns (uint256 _realPayAmount, uint256 _realSaleAmount, uint256 _refundAmount) {
        ...
        console.log("openSaleUserAmount userOpen.depositAmount: %s %s %s", user, userOpen.depositAmount, userOpen.join );
        ...
        console.log("openSalePossible: %s , realPayAmount %s , depositAmount %s", openSalePossible, realPayAmount, depositAmount );
        ...
        console.log("realPayAmount: %s , realSaleAmount %s , returnAmount %s", realPayAmount, realSaleAmount, returnAmount );
    ...
    function totalSaleUserAmount(address user) public view returns (uint256 _realPayAmount, uint256 _realSaleAmount, uint256 _refundAmount) {
        ...
        console.log("totalSaleUserAmount userEx.join: %s , payAmount: %s ,  saleAmount %s ",userEx.join, userEx.payAmount, userEx.saleAmount);

        if(userEx.join){
            (uint256 realPayAmount, uint256 realSaleAmount, uint256 refundAmount) = openSaleUserAmount(user);
            console.log("totalSaleUserAmount join:true realPayAmount: %s , realSaleAmount %s , returnAmount %s", realPayAmount.add(userEx.payAmount), realSaleAmount.add(userEx.saleAmount), refundAmount);

            return ( realPayAmount.add(userEx.payAmount), realSaleAmount.add(userEx.saleAmount), refundAmount);
        }else {

            console.log("totalSaleUserAmount join:false");
            return openSaleUserAmount(user);
        }
    }

    function totalOpenSaleAmountView() public view returns (uint256){
    ...
    function totalOpenPurchasedAmountView() public view returns (uint256){
        uint256 _calculSaleToken = calculSaleToken(totalDepositAmount);
        uint256 _totalAmount = totalExpectOpenSaleAmountView();
    ...
    function claim() external override {
        ...
        console.log("claim: payAmount: %s , saleAmount %s , refundAmount %s ", realPayAmount, realSaleAmount, refundAmount);

        require(realSaleAmount > 0, "PublicSale: no purchase amount");

        uint256 reward = calculClaimAmount(msg.sender);
        require(reward > 0, "PublicSale: no claimable amount");
        require(userClaim.claimAmount.add(reward) <= realSaleAmount, "PublicSale: already claimed");
        require(
            saleToken.balanceOf(address(this)) >= reward,
            "PublicSale: dont have saleToken in pool"
        );
        console.log("claim: reward: %s   ", reward);
        console.log("claim: userClaim.claimAmount: %s   ", userClaim.claimAmount);
