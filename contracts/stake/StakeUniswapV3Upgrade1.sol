    ...
    DSMath
{
    ...
    function claim(uint256 tokenId) external override {
        ...
        require(!_depositTokens.claimLock && !_depositTokens.withdraw, "StakeUniswapV3: claiming");
    ...
    function withdraw(uint256 tokenId) external override {
        ...
        require(
            !_depositTokens.withdraw && !_depositTokens.claimLock,
    ...
    {
        LibUniswapV3Stake.StakeLiquidity storage _depositTokens = depositTokens[params.tokenId];
        ...
        LibUniswapV3Stake.StakedTokenAmount storage _stakedCoinageTokens =
            stakedCoinageTokens[params.tokenId];
        ...
        emit IncreasedLiquidity(msg.sender, params.tokenId, liquidity, amount0, amount1);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        LibUniswapV3Stake.StakeLiquidity storage _depositTokens = depositTokens[params.tokenId];
        require(msg.sender == _depositTokens.owner, 'not owner');
        require(_depositTokens.liquidity > params.liquidity, 'insufficient liquidity');

        require(!_depositTokens.claimLock && !_depositTokens.withdraw, "StakeUniswapV3: in process");
        _depositTokens.claimLock = true;

        uint128 positionLiquidity = params.liquidity;
        miningCoinage();
        (amount0, amount1) = nonfungiblePositionManager.decreaseLiquidity(params);

        _depositTokens.liquidity -= positionLiquidity;
        totalStakedAmount = totalStakedAmount.sub(uint256(positionLiquidity));

        LibUniswapV3Stake.StakedTotalTokenAmount storage _userTotalStaked =
            userTotalStaked[msg.sender];
        _userTotalStaked.totalDepositAmount = _userTotalStaked
            .totalDepositAmount
            .sub(uint256(positionLiquidity));
        LibUniswapV3Stake.StakedTokenAmount storage _stakedCoinageTokens =
            stakedCoinageTokens[params.tokenId];
        _stakedCoinageTokens.amount = _stakedCoinageTokens.amount.sub(uint256(positionLiquidity));

        //mint coinage of user amount
        IAutoRefactorCoinageWithTokenId(coinage).burn(
            msg.sender,
            params.tokenId,
            uint256(positionLiquidity).mul(10**9)
        );

        _depositTokens.claimLock = false;

        emit DecreaseLiquidity(msg.sender, params.tokenId, params.liquidity, amount0, amount1);
    }
    /*
    /// when staking, hooking
