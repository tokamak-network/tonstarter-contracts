...
contract StakeRegistry is AccessControl {
    ...
    modifier onlyOwner() {
        require(hasRole(ADMIN_ROLE, msg.sender), "StakeRegistry: Caller is not an admin");
        _;
    }
    ...
    function addVault(
        ...
        onlyOwner
    {
        require(vaultNames[_vault] == ZERO_HASH || vaults[_vaultName] == address(0),
    ...
    returns (bool valid)
    {
        require(phases[_pahse].length > 0 && vaultNames[_vault] != ZERO_HASH,
