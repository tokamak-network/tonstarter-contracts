...
describe(" UniswapV3 LP Staking", function () {
  ...
  describe('# 3. Phase 2 : Staking  ', async function () {
     it("1. Stake In Contract", async function () {
        console.log('vault',vaultAddress, 'StakeUniswapV3:', stakeContractAddress);
        let ethers = hre.ethers;

        if(stakeContractAddress!=null ){
            // const StakeContractArtifact = await artifacts.readArtifact("StakeUniswapV3");
            // const contract = new ethers.Contract(stakeContractAddress, StakeContractArtifact.abi, ethers.provider);
            const contract = await ethers.getContractAt("StakeUniswapV3", stakeContractAddress);
            ...
            await contract.connect(tester).stake(
              tokenId_First,
              deadline,
            );
