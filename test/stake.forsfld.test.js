...
describe('StakeForSFLD ', function () {
  ...
  it('mint fld to tester for testing  ', async function () {
    ...
    testStakingUsers.forEach(async(user) => {
      await fld.mint(user, utils.parseUnits(sendAmountForTest, 18), { from: defaultSender });
    });
  ...
  it('create StakeForSFLD  ', async function () {
    ...
    stakeForSFLDs = await ico20Contracts.createStaekForSFLD(toBN(testStartBlock), defaultSender );

    for(let i = 0; i < testPeriodBlocks.length; i++){
      await stakeForSFLDs.addRewardRatio(toBN(testPeriodBlocks[i]), toBN(testRewardRatio[i]),{ from: defaultSender });
      //let rewardRatio= await stakeForSFLDs.rewardRatio(toBN(testPeriodBlocks[i]));
      expect(await stakeForSFLDs.rewardRatio(toBN(testPeriodBlocks[i]))).to.be.bignumber.equal(toBN(testRewardRatio[i]));
    }
  ...
  it('stake  ', async function () {
    ...
    for(let index = 0; index < testStakingUsers.length; index++){
      ...
      let rewardBlocks0 = await stakeForSFLDs.rewardRatio(toBN(testPeriodBlocks[0]));
      console.log('rewardBlocks: ', testPeriodBlocks[0], rewardBlocks0.toString());
      let rewardBlocks1 = await stakeForSFLDs.rewardRatio(toBN(testPeriodBlocks[1]));
      console.log('rewardBlocks: ', testPeriodBlocks[1], rewardBlocks1.toString());
      ...
      for(let i = 0; i < testPeriodBlocks.length; i++){
        ...
        let sig = await getSignature(ownerPrivate, owner, spender, value,  deadline);

        console.log('value ', value.toString());
        //console.log('testPeriodBlocks[i] ', i, testPeriodBlocks[i]);
        // console.log('deadline ', deadline);
        // console.log('sig ', sig);
        let tx = await stakeForSFLDs.stake(value, toBN(testPeriodBlocks[i]), toBN(deadline), sig, { from: owner });
  ...
  it('claim ', async function () {
    ...
    for(let index = 0; index < testStakingUsers.length; index++){
      ...
      for(let i = 0; i < testPeriodBlocks.length; i++){
        ...
        if (logFlag) console.log(`\sfldBalance Pre claim:`, utils.formatUnits(sfldBalance.toString(), 18) );

        await stakeForSFLDs.claim({ from: user });

        sfldBalance = await sfld.balanceOf(user);
        if (logFlag) console.log(`\sfldBalance After claim:`, utils.formatUnits(sfldBalance.toString(), 18) );
  ...
  it('withdraw  ', async function () {
    ...
    for(let index = 0; index < testStakingUsers.length; index++){
      ...
      if (logFlag) console.log(`\n fldBalance :`, utils.formatUnits(fldBalance.toString(), 18));

      let sfldBalance = await sfld.balanceOf(user);
      if (logFlag) console.log(`\n sfldBalance :`, utils.formatUnits(sfldBalance.toString(), 18));
      ...
      let ownerPrivate = testStakingUserPrivateKeys[index];
      let spender = stakeForSFLDs.address;
      let sig = await getSignature(ownerPrivate, owner, spender, sfldBalance,  deadline);

      await stakeForSFLDs.withdraw(sfldBalance, toBN(deadline), sig,  { from: user });

      fldBalance = await fld.balanceOf(user);
      if (logFlag) console.log(`\n fldBalance After claim:`,  utils.formatUnits(fldBalance.toString(), 18));
  ...
  async function logUserStaked (_contract, _user, username) {
    ...
    console.log('rewardPerBlock', utils.formatUnits(userStaked.rewardPerBlock.toString(), 18));
