...
describe("Sale", () => {
    ...
    describe("Sale", () => {
        ...
        describe("openSale Sale", () => {
            ...
            it("openSale function", async () => {
                ...
                expect(Number(tx4.totalClaimReward)).to.be.equal(300000)
                let totaldeposit = await saleContract.totalDepositAmount()
                expect(Number(totaldeposit)).to.be.equal(2000)
                ...
                expect(afterTON).to.be.equal(2000)

                account1After = Number(await getToken.balanceOf(account1.address))
                account2After = Number(await getToken.balanceOf(account2.address))
                account3After = Number(await getToken.balanceOf(account3.address))
                account4After = Number(await getToken.balanceOf(account4.address))
                let diff1 = account1Before - account1After
                let diff2 = account2Before - account2After
                let diff3 = account3Before - account3After
                let diff4 = account4Before - account4After
                expect(diff1).to.be.equal(100)
                expect(diff2).to.be.equal(200)
                expect(diff3).to.be.equal(300)
                expect(diff4).to.be.equal(400)
                ...
                expect(Number(tx6.totalClaimReward)).to.be.equal(320000)
                let tx7 = await saleContract.usersClaim(account3.address)
                expect(Number(tx7.totalClaimReward)).to.be.equal(520000)
    ...
    describe("claim test", () => {
        ...
        it("claim period = 6, claim call the account1, account2", async () => {
            ...
            let claimAmount2 = await saleContract.usersClaim(account2.address)
            expect(Number(claimAmount2.claimAmount)).to.be.equal(Number(claimAmount2.periodReward * 2))
        ...
        it("claim period end, claim call the account1, account3, account4, account6", async () => {
            ...
            let expectClaim4 = Number(await saleContract.calculClaimAmount(account6.address))

            let claimAmount1 = await saleContract.usersClaim(account3.address)
            expect(Number(claimAmount1.claimAmount)).to.be.equal(Number(claimAmount1.periodReward * 3))
            let claimRemainAmount1 = Number(claimAmount1.totalClaimReward) - Number(claimAmount1.claimAmount)
            expect(claimRemainAmount1).to.be.equal(expectClaim2)

            let claimAmount2 = await saleContract.usersClaim(account4.address)
            expect(Number(claimAmount2.claimAmount)).to.be.equal(Number(claimAmount2.periodReward * 4))
            let claimRemainAmount2 = Number(claimAmount2.totalClaimReward) - Number(claimAmount2.claimAmount)
            expect(claimRemainAmount2).to.be.equal(expectClaim3)

            let claimAmount3 = await saleContract.usersClaim(account6.address)
            expect(Number(claimAmount3.totalClaimReward)).to.be.equal(expectClaim4)
            ...
            expect(Number(tx7)).to.be.equal(Number(claimAmount3.totalClaimReward))
        ...
        it("data check", async () => {
            ...
            expect(Number(tx11.saleAmount)).to.be.equal(300000)
            let tx12 = await saleContract.usersOpen(account4.address)
            expect(Number(tx12.saleAmount)).to.be.equal(400000)
