...
describe("Sale", () => {
    ...
    //total 구매 및 결과
    //account1 -> 160,000 DOC -> 160 TON 구매 100 TON 환불
    //account2 -> 320,000 DOC -> 320 TON 구매 200 TON 환불
    //account3 -> 520,000 DOC -> 520 TON 구매 300 TON 환불
    //account4 -> 600,000 DOC -> 700 TON 구매 400 TON 환불
    //account6 -> 300,000 DOC -> 300 TON 구매
    ...
    describe("Initialize PublicSale", () => {
        ...
        it("Initialize PublicSale", async function () {
            ...
            await getToken.connect(saleTokenOwner).transfer(account1.address, basicAmount)
            await getToken.connect(saleTokenOwner).mint(account2.address, basicAmount)
            await getToken.connect(saleTokenOwner).mint(account3.address, basicAmount)
            await getToken.connect(saleTokenOwner).mint(account4.address, basicAmount)
            await getToken.connect(saleTokenOwner).mint(account6.address, basicAmount)
        });
    });
