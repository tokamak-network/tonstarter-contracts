...
describe("Sale", () => {
    ...
    let refund5 = ethers.utils.parseUnits("5", 18);
    let refund10 = ethers.utils.parseUnits("10", 18);
    ...
    describe("Sale", () => {
        describe("exclusiveSale Sale", () => {
            ...
            it("exclusiveSale after exclusive startTime", async () => {
                ...
                await getToken.connect(account1).approveAndCall(saleContract.address, refund5, 0);
                let tx = await saleContract.usersEx(account1.address)
                expect(Number(tx.payAmount)).to.be.equal(Number(refund5))

                await getToken.connect(account2).approveAndCall(saleContract.address, refund10, 0);
                let tx2 = await saleContract.usersEx(account2.address)
                expect(Number(tx2.payAmount)).to.be.equal(Number(refund10))
                
                await getToken.connect(account3).approveAndCall(saleContract.address, refund5, 0);
                let tx3 = await saleContract.usersEx(account3.address)
                expect(Number(tx3.payAmount)).to.be.equal(Number(refund5))
                          
                await getToken.connect(account6).approveAndCall(saleContract.address, refund10, 0);
                let tx5 = await saleContract.usersEx(account6.address)
                expect(Number(tx5.payAmount)).to.be.equal(Number(refund10))
                ...
                let big30 = ethers.utils.parseUnits("30", 18);
                let big30000 = ethers.utils.parseUnits("30000", 18);
                let tx6 = Number(await saleContract.totalExPurchasedAmount())
                expect(tx6).to.be.equal(Number(big30))
                let tx7 = Number(await saleContract.totalExSaleAmount())
                expect(tx7).to.be.equal(Number(big30000))
            })
        })
        ...
        describe("openSale Sale", () => {
            ...
            it("deposit after depositTime", async () => {
                ...
                await getToken.connect(account1).approveAndCall(saleContract.address, refund5, 0);
                await getToken.connect(account3).approveAndCall(saleContract.address, refund5, 0);
                await getToken.connect(account4).approveAndCall(saleContract.address, refund10, 0);
                ...
                expect(Number(tx.depositAmount)).to.be.equal(Number(refund5))
                let tx3 = await saleContract.usersOpen(account3.address)
                expect(Number(tx3.depositAmount)).to.be.equal(Number(refund5))
                let tx4 = await saleContract.usersOpen(account4.address)
                expect(Number(tx4.depositAmount)).to.be.equal(Number(refund10))
            })
        })
    })
    ...
    describe("claim test", () => {
        ...
        it("claim claimTime1, claim call the account1, account2", async () => {
            let check = await saleContract.hardcapCalcul();
            ...
            let beforeTONaccount1 = Number(await getToken.balanceOf(account1.address));
            // console.log("beforeTONaccount1 : ", beforeTONaccount1);
            await saleContract.connect(account1).claim()
            let tx2 = await saleContract.usersClaim(account1.address)
            expect(Number(tx2.claimAmount)).to.be.equal(0)
            let afterTONaccount1 = Number(await getToken.balanceOf(account1.address))
            // console.log("afterTONaccount1 : ", afterTONaccount1);
            let diff1 = afterTONaccount1 - beforeTONaccount1
            expect(Number(diff1)).to.be.equal(Number(refund10))

            let beforeTONaccount2 = Number(await getToken.balanceOf(account2.address))
            await saleContract.connect(account2).claim()
            let afterTONaccount2 = Number(await getToken.balanceOf(account2.address))
            let diff2 = afterTONaccount2 - beforeTONaccount2
            expect(Number(diff2)).to.be.equal(Number(refund10))
        })
        ...
        it("claim claimTime2, claim call the account3, account4, account6", async () => {
            let beforeTONaccount3 = Number(await getToken.balanceOf(account3.address));
            await saleContract.connect(account3).claim()
            let afterTONaccount3 = Number(await getToken.balanceOf(account3.address))
            let diff3 = afterTONaccount3 - beforeTONaccount3
            expect(Number(diff3)).to.be.equal(Number(refund10))

            let beforeTONaccount4 = Number(await getToken.balanceOf(account4.address))
            await saleContract.connect(account4).claim()
            let afterTONaccount4 = Number(await getToken.balanceOf(account4.address))
            let diff4 = afterTONaccount4 - beforeTONaccount4
            expect(Number(diff4)).to.be.equal(Number(refund10))

            let beforeTONaccount6 = Number(await getToken.balanceOf(account6.address))
            await saleContract.connect(account6).claim()
            let afterTONaccount6 = Number(await getToken.balanceOf(account6.address))
            let diff6 = afterTONaccount6 - beforeTONaccount6
