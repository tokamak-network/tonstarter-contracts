...
describe("DeveloperVault", function () {
  ...
  before(async function () {
    ...
    const DeveloperVaultContract = await ethers.getContractFactory(
      "DeveloperVault"
    );
    DeveloperVault = await DeveloperVaultContract.connect(deployer).deploy(
      admin.address
    );
  ...
  it("should claim and fail 1", async function () {
    await expect(DeveloperVault.connect(dev1).claimReward()).to.be.revertedWith(
      "Period for reward has not been reached"
    );
    expect(await FLD.balanceOf(dev1.address)).to.equal(0);

    await expect(DeveloperVault.connect(dev2).claimReward()).to.be.revertedWith(
      "Period for reward has not been reached"
    );
    expect(await FLD.balanceOf(dev2.address)).to.equal(0);

    await expect(DeveloperVault.connect(dev3).claimReward()).to.be.revertedWith(
      "Period for reward has not been reached"
    );
  ...
  it("should claim and fail 2", async function () {
    const currentBlockTime =
      parseInt(startRewardBlock) + parseInt(rewardPeriod) - 2; // too early to claim reward for second round
    await time.advanceBlockTo(currentBlockTime);
  ...
  it("should claim and succeed 2", async function () {
    await time.advanceBlockTo(
      parseInt(startRewardBlock) + parseInt(rewardPeriod)
    );
  ...
  it("should claim and succeed 3", async function () {
    await time.advanceBlockTo(
      parseInt(startRewardBlock) + 2 * parseInt(rewardPeriod)
    );
  ...
  it("should claim and fail 3", async function () {
    const currentBlockTime =
      parseInt(startRewardBlock) + 3 * parseInt(rewardPeriod);
    await time.advanceBlockTo(currentBlockTime);
