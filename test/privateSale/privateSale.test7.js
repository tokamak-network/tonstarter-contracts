...
describe("PrivateSale", function () {
    ...
    let claim1Percents = 20;
    let claim2Percents = 20;
    let claim3Percents = 20;
    let claim4Percents = 20;
    let claim5Percents = 20;
    ...
    describe("# 4. whitelist setting", async function () {
        it("buy before saleTime", async () => {
            let buy1 = privateSaleContract.connect(tester1.account).buy(
                tester1.account.address,
                tester1.claimAccount.address,
                tester1.buyAmount
            )
            await expect(buy1).to.be.revertedWith("privaSale period end")
            let buy2 = privateSaleContract.connect(tester2.account).buy(
                tester2.account.address,
                tester2.claimAccount.address,
                tester2.buyAmount
            )
            await expect(buy2).to.be.revertedWith("privaSale period end")
            let buy3 = privateSaleContract.connect(tester3.account).buy(
                tester3.account.address,
