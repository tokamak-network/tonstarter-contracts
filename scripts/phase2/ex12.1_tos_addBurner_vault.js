...
async function main() {
  ...
  let res = await tos.grantRole(BURNER, process.env.PHASE2_LP_VAULT_ADDRESS);
  console.log("tos isAdmin", process.env.PHASE2_LP_VAULT_ADDRESS,  res );
  await res.wait();

  let res1 = await tos.hasRole(BURNER, process.env.PHASE2_LP_VAULT_ADDRESS);
