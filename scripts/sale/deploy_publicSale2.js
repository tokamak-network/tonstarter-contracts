...
async function deployLogic2() {
    const [deployer] = await ethers.getSigners()
    console.log("Deploying contract with the account :", deployer.address)

    const LibPublicSale2 = await ethers.getContractFactory("LibPublicSale2");
    let libPublicSale2 = await LibPublicSale2.connect(deployer).deploy();
    await libPublicSale2.deployed();

    console.log("libPublicSale2 : ", libPublicSale2.address);
    ...
    const saleContract = await PublicSale2.connect(deployer).deploy()
    console.log("saleContract Address: ", saleContract.address)
    await saleContract.deployed();
    console.log("finish");
}
