...
async function deployMain(defaultSender) {
  ...
  const stakeTON = await ethers.getContractAt("StakeTONProxy2", process.env.PHASE1_TON_3_ADDRESS);

  let isAdmin = await stakeTON.isAdmin(deployer.address);
  console.log("isAdmin PHASE1_TON_3_ADDRESS", deployer.address, isAdmin);

  let tx1 = await stakeTON.setImplementation2(StakeTONUpgrade2, 0, true);
  console.log("setImplementation2 PHASE1_TON_3_ADDRESS StakeTONUpgrade2 ", tx1.hash);
  printGasUsedOfUnits('setImplementation2 PHASE1_TON_3_ADDRESS StakeTONUpgrade2',tx1);

  let tx2 = await stakeTON.setImplementation2(StakeTONUpgrade3, 1, true);
  console.log("setImplementation2 PHASE1_TON_3_ADDRESS StakeTONUpgrade3", tx2.hash);
  printGasUsedOfUnits('setImplementation2 PHASE1_TON_3_ADDRESS StakeTONUpgrade3',tx2);
  ...
  console.log("setSelectorImplementations2 PHASE1_TON_3_ADDRESS withdraw", tx3.hash);
  printGasUsedOfUnits('setSelectorImplementations2 PHASE1_TON_3_ADDRESS withdraw',tx3);
