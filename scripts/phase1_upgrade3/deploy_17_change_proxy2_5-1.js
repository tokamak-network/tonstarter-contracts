...
async function deployMain(defaultSender) {
  ...
  let tx1 = await proxyContract.connect(deployer).grantRole(process.env.PHASE1_TON_5_ADDRESS, keccak256("ADMIN"), deployer.address );
  console.log("grantRole PHASE1_TON_5_ADDRESS", tx1.hash);
  printGasUsedOfUnits('grantRole PHASE1_TON_5_ADDRESS',tx1);

  const stakeEntry = await ethers.getContractAt("Stake1Logic", proxy);
  console.log("stakeEntry:", stakeEntry.address);

  let tx2 = await stakeEntry.connect(deployer).upgradeStakeTo(process.env.PHASE1_TON_5_ADDRESS, StakeTONProxy2);
  console.log("upgradeStakeTo PHASE1_TON_5_ADDRESS StakeTONProxy2", tx2.hash);
  printGasUsedOfUnits('upgradeStakeTo PHASE1_TON_5_ADDRESS StakeTONProxy2',tx2);
