...
async function deployMain(defaultSender) {
  ...
  const StakeTONUpgrade3 = await ethers.getContractFactory("StakeTONUpgrade3");

  let deployInfo = {name:'', address:''};

  const stakeTONUpgrade3 = await StakeTONUpgrade3.deploy();
  let tx  = await stakeTONUpgrade3.deployed();
  console.log("StakeTONUpgrade3:", stakeTONUpgrade3.address);

  deployInfo = {
    name: "StakeTONUpgrade3",
    address: stakeTONUpgrade3.address
  }
  if(deployInfo.address != null && deployInfo.address.length > 0  ){
    save(process.env.NETWORK, deployInfo);
  }
  printGasUsedOfUnits('StakeTONUpgrade3 Deploy',tx);
  return null;
}
