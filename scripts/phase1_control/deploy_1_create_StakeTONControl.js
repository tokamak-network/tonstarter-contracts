...
async function deployMain(defaultSender) {
  ...
  const StakeTONControl = await ethers.getContractFactory("StakeTONControl");

  let deployInfo = {name:'', address:''};

  const stakeTONControl = await StakeTONControl.deploy();
  let tx  = await stakeTONControl.deployed();
  console.log("StakeTONControl:", stakeTONControl.address);

  deployInfo = {
    name: "StakeTONControl",
    address: stakeTONControl.address
  }
  if(deployInfo.address != null && deployInfo.address.length > 0  ){
    save(process.env.NETWORK, deployInfo);
  }
  printGasUsedOfUnits('StakeTONControl Deploy',tx);
  return null;
}
