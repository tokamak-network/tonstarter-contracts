...
async function deployMain(defaultSender) {
  ...
  let tx1 = await stakeTONControl.connect(deployer).setInfo(
    ton,
    wton,
    tostoken,
    DepositManager,
    SeigManager,
    process.env.TOKAMAK_LAYER2,
    5
    );
  console.log("setInfo StakeTONControl ", tx1.hash);
  printGasUsedOfUnits('setInfo StakeTONControl',tx1);
