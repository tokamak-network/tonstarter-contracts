...
const { findAccount, encodePath } = require("../tasks/utils");
...
async function main() {
 ...
 // console.log('ethers.provider',ethers.provider );
  ...
  let path = encodePath([wton, uniswapWeth, tos], [3000,3000]);
...
console.log('oneWTON',oneWTON.toString());
  ...
  let deadline = Date.now()/1000 + 200000;
...
//   console.log('params',params2 );
  ...
  const tx2 = await uniswapRouter1.connect(users).exactInput(params1, {
    gasLimit: 10000000,
    gasPrice: 5000000000,
  });
  console.log('exactInput',tx2.hash);
