...
async function deployMain(defaultSender) {
  ...
  let _isAdmin = await liquidityMiningVault.isAdmin(user2.address);
  console.log("liquidityMiningVault isAdmin:  ", user2.address, _isAdmin );

  if(_isAdmin){
    let tx = await liquidityMiningVault.connect(user2).claimTOS(
