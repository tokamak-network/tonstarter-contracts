...
async function deployMain(defaultSender) {
  ...
  const StakeTONUpgrade2 = await ethers.getContractFactory("StakeTONUpgrade2");

  let deployInfo = {name:'', address:''};

  const stakeTONUpgrade2 = await StakeTONUpgrade2.deploy();
  let tx  = await stakeTONUpgrade2.deployed();
  console.log("StakeTONUpgrade2:", stakeTONUpgrade2.address);

  deployInfo = {
    name: "StakeTONUpgrade2",
    address: stakeTONUpgrade2.address
  }
  if(deployInfo.address != null && deployInfo.address.length > 0  ){
    save(process.env.NETWORK, deployInfo);
  }
  printGasUsedOfUnits('StakeTONUpgrade2 Deploy',tx);
  return null;
}
